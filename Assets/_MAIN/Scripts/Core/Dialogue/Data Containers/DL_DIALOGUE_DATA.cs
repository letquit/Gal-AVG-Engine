    ...
    public class DL_DIALOGUE_DATA
    {
        ...
        private List<DIALOGUE_SEGMENT> RipSegments(string rawDialogue)
        {
            ...
            for (int i = 0; i < matches.Count; i++)
            {
                ...
                // 设置起始信号类型
                segment.startSignal =
                    (DIALOGUE_SEGMENT.StartSignal)Enum.Parse(typeof(DIALOGUE_SEGMENT.StartSignal),
                        signalSplit[0].ToUpper());

                // 若存在延迟参数，则尝试解析为浮点数
                if (signalSplit.Length > 1)
                    float.TryParse(signalSplit[1], NumberStyles.Any,
                        CultureInfo.InvariantCulture, out segment.signalDelay);
